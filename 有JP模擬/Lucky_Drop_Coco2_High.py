...
def draw_free_game_mini_slot(rng, jackpot_pool, bet_amount, multiplier):
    ...
    if roll < 0.00221:
        jp_share = 0.5 * jackpot_pool * (bet_amount / max_payout)
        reward = (jp_share + bet_amount * 200) * multiplier
        label = "Super JP / X 200 獎"
    elif roll < 0.01135:
        jp_share = 0.1 * jackpot_pool * (bet_amount / max_payout)
        reward = (jp_share + bet_amount * 100) * multiplier
        label = "Lucky JP / X 100 獎"
    elif roll < 0.07568:
        reward = bet_amount * 50 * multiplier
        label = "X 50 獎"
        jp_share = 0.0
    elif roll < 0.19027:
        ...
        reward = bet_amount * 3 * multiplier
        label = "X 3 獎"
        jp_share = 0.0
...
for i in range(total_rounds // batch_size):
    ...
    if total_so_far % save_interval == 0:
        ...
        filename = f"lucky_drop_Coco2_High_95.5_part{part_id}.xlsx"
